...
def predict(image):
    ...
    result = "Predicted: " + str(predicted_digit) + "  (" + str(round(confidence_pct, 1)) + "% confidence)"
...
with gr.Blocks(title="Digit Recognizer") as demo:
    ...
    with gr.Row():
        with gr.Column():
            canvas = gr.Sketchpad(label="Draw Here", type="numpy", image_mode="RGB", canvas_size=(280, 280), brush=gr.Brush(colors=["#000000"], default_size=20))
            with gr.Row():
...
if __name__ == "__main__":
    demo.launch(server_name="0.0.0.0", server_port=7860, inbrowser=True, share=False, theme=gr.themes.Soft())
